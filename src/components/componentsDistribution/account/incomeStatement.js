...
import loadAllLogs from "../../../redux/reduxDistribution/actions/logs/getLogsAction"; // Import par défaut
...
console.log("Total logs count:", total);
  ...
  const columns = [
    ...
    { title: "Utilisateur", dataIndex: ["user", "userName"], key: "user" },
  ...
  useEffect(() => {
    dispatch(loadAllLogs({ page: 1, limit: count, startdate: startDate, enddate: endDate }));
...
console.log("Logs from state:", logs);
      ...
      <Card>
        <div className="card-title d-flex flex-column flex-md-row align-items-center justify-content-md-center mt-1 py-2">
          <Form onFinish={onSearchFinish} layout={"inline"}>
            <Form.Item name="user">
              <Select placeholder="Utilisateur" style={{ width: 200 }} allowClear>
            ...
            <Form.Item>
              <Button onClick={() => setLoading(true)} loading={loading} type="primary" htmlType="submit">
                <SearchOutlined />
              </Button>
            </Form.Item>
          </Form>
  ...
  pagination={{
    ...
    onChange: (page, limit) => {
      dispatch(
        loadAllLogs({ page, limit, startdate: startDate, enddate: endDate, user })
      );
    },
  }}
  columns={columns}
